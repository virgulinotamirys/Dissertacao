...
public class HapticDentistPick : MonoBehaviour
{
    //public int teethID = 0;        //!< index of the button assigned to grabbing.  Defaults to the first button
    //public bool TeethActsAsToggle = false; //!< Toggle button? as opposed to a press-and-hold setup?  Defaults to off.
    ...
    //private bool teethStatus = false;          //!< Is the button currently pressed?
    ...
    /*void FixedUpdate()
    {
        bool newteethStatus = hapticDevice.GetComponent<HapticPlugin>().Buttons[teethID] == 1;
        bool oldteethStatus = teethStatus;
        teethStatus = newteethStatus;


        if (oldteethStatus == false && newteethStatus == true)
        {
            if (TeethActsAsToggle)
            {
                if (grabbing)
                    release();
                else
        ...
        if (oldteethStatus == true && newteethStatus == false)
        {
            if (TeethActsAsToggle)
            {
                //Do Nothing
    ...
    private void hapticTouchEvent(bool isTouch)
    {
        ...
        if (physicsToggleStyle == PhysicsToggleStyle.onTouch)
        {
            ...
            /*Ray inputRay = Camera.main.ScreenPointToRay(hapticDevice.GetComponent<HapticPlugin>().proxyPositionRaw);
            RaycastHit hit;
            ...
            if (Physics.Raycast(inputRay, out hit))
            {
                ...
                if (deformer)
                {
                    ...
                    point += hit.normal * hapticDevice.GetComponent<HapticPlugin>().PhysicsForceDamping;
                    deformer.AddDeformingForce(point, hapticDevice.GetComponent<HapticPlugin>().PhysicsForceStrength);
                }
            }*/
